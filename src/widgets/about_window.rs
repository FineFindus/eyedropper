...
impl EyedropperAbout {
    pub fn show<A: IsA<Application> + AdwApplicationExt, W: IsA<Window> + GtkWindowExt>(
        ...
        let about_window = AboutWindow::builder()
            ...
            .developers(DEVELOPERS.iter().map(|dev| dev.to_string()).collect())
            .designers(DESIGNERS.iter().map(|dev| dev.to_string()).collect())
            .artists(ARTISTS.iter().map(|dev| dev.to_string()).collect())
    ...
    fn changelog() -> String {
        ...
        changelog.push_str(&pgettext(
            ...
            "<p>This release extends the current available features.</p><p> Features:</p>\n<ul><li> The name of the color, specified by w3c color keywords and the xkcd color survey, can now be shown</li><li> A new Palette dialog showing different color palettes</li><li> Show the CIELCh color format</li><li> Using the new AdwAboutWindow</li><li> Added new translations</li><li>Fixed minor issues</li></ul>",
        ));
